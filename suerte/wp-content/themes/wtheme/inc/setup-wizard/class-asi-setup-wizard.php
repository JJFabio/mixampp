...
public function setup_wizard_header() {
    ...
    <html <?php language_attributes(); ?>>
        ...
        <body class="asi-setup wp-core-ui">
            ...
            ?>
            <a class="asi-return-to-dashboard" href="<?php echo esc_url(admin_url()); ?>"><?php echo esc_html(__('Return to your dashboard', ASITHEME_SLUG)); ?></a>
        </body>
    </html>
    <?php
}
            ...
            <div class="inside">
                ...
                <div class="asi-wizard-enable" id="asi-setup-create-header-widgets">
                    <label>
                        ...
                        <?php echo esc_html(__('Create the cart widget in the header', ASITHEME_SLUG)); ?>
                    </label>
                </div>
                <div class="asi-wizard-enable" id="asi-setup-create-footer-widgets">
                    <label>
                        ...
                        <?php echo esc_html(__('Create footer widgets', ASITHEME_SLUG)); ?>
                    </label>
                </div>
    ...
    public function asi_setup_content_save() {
        ...
        if ($frontpage) {
            ...
            if ($frontpage_id) {
                ...
                foreach ($items as $item) {
                    $filename = ASITHEME_CDN_PATH . '/assets/images/' . $item;
                ...
                $widgets = array(
                    array(
                        ...
                        'data' => array(
                            ...
                            'text' => 'Adipiscing elit. Sed eu eros dui. Aliquam erat. Sed eu eros dui.',
                            'text_align' => 'right',
                            'text_color' => '#000',
                            'button_text' => __('Buy', ASITHEME_SLUG),
                            'button_chooser' => 'button_link',
                            'button_link' => get_home_url(),
                            'button_link_widget' => '',
                            'button_target' => 0,
                            'background_image' => $images['widget-main-1.jpg'],
                            'background_opacity' => '0.5',
                        )
                    ),
                    array(
                        'id_base' => 'asitheme_widget_woocommerce_categories',
                        'data' => array(
                            'noparents' => 1,
                            'counter' => 1,
                            'images' => 1,
                    ...
                    array(
                        ...
                        'data' => array(
                            'title' => '',
                            'text' => 'Adipiscing elit. Sed eu eros dui. Aliquam erat.',
                            'text_align' => 'left',
                            'text_color' => '#ffffff',
                            'button_text' => '',
                            'button_chooser' => '',
                            ...
                            'button_link_widget' => '',
                            'button_target' => 0,
                            'background_image' => $images['widget-main-2.jpg'],
                            'background_opacity' => '0.5',
                        )
                    ),
                );
        ...
        if ($footer_widgets) {
            ...
            $widgets_1 = array(
                'id_base' => 'custom_html',
                'data' => array(
                    'title' => __('Legal issues', ASITHEME_SLUG),
                    'content' => '<a href="' . CHILD_THEME_AUTHORURI . '" target="_blank">' . __('Legal warning', ASITHEME_SLUG) . '</a><br/>
                                    <a href="' . CHILD_THEME_AUTHORURI . '" target="_blank">' . __('Privacy Policy', ASITHEME_SLUG) . '</a><br/>
                                    <a href="' . CHILD_THEME_AUTHORURI . '" target="_blank">' . __('General conditions of online purchase', ASITHEME_SLUG) . '</a><br/>
                                    <a href="' . CHILD_THEME_AUTHORURI . '" target="_blank">' . __('Cookies policy', ASITHEME_SLUG) . '</a>',
                )
            );
            ...
            $widgets_2 = array(
                'id_base' => 'custom_html',
                'data' => array(
                    'title' => __('Client area', ASITHEME_SLUG),
                    'content' => '<a href="' . CHILD_THEME_AUTHORURI . '" target="_blank">' . __('Log in', ASITHEME_SLUG) . '</a><br/>
                                    <a href="' . CHILD_THEME_AUTHORURI . '" target="_blank">' . __('Contact', ASITHEME_SLUG) . '</a><br/>
                                    <a href="' . CHILD_THEME_AUTHORURI . '" target="_blank">' . __('Faqs', ASITHEME_SLUG) . '</a>',
                )
            );
            ...
            <div class="inside">
                ...
                <p class="asi-wizard-enable" id="asi-setup-header-search">
                    <label>
                        ...
                        <?php echo esc_html(__('Show search engine as in the demo?', ASITHEME_SLUG)); ?>
                    </label>
                </p>
            </div>
    ...
    public function asi_setup_header_save() {
        ...
        if ($search) {
            set_theme_mod(ASITHEME_SLUG . '_header_search', true);
        } else {
            set_theme_mod(ASITHEME_SLUG . '_header_search', false);
        }
    ...
    public function asi_setup_import_save() {
        ...
        if ($posts) {
            ...
            foreach ($ids as $id) {
                ...
                $filename = ASITHEME_CDN_PATH . '/assets/images/post-asithemes-' . $id . '.jpg';
                $filename_upload = $wp_upload_dir['url'] . '/' . basename($filename);
                $filename_upload_path = $wp_upload_dir['path'] . '/' . basename($filename);
                copy($filename, $filename_upload_path);
                $filetype = wp_check_filetype(basename($filename), null);
        ...
        if ($products && is_plugin_active('woocommerce/woocommerce.php')) {
            ...
            if (!$exist_products) {
                ...
                /* Create test terms */
                $product_cat_ids_front_page = array();
                $term_ids = array(8, 7, 6, 5, 4, 3, 2, 1);
                foreach ($term_ids as $term_id) {

                    /* Insert term */
                    $term = wp_insert_term(__('Category', ASITHEME_SLUG) . ' ' . $term_id, 'product_cat', array());
                    array_push($product_cat_ids_front_page, $term['term_id']);

                    /* Upload image */
                    $filename = ASITHEME_CDN_PATH . '/assets/images/product-cat-asithemes-' . $term_id . '.jpg';
                    $filename_upload = $wp_upload_dir['url'] . '/' . basename($filename);
                    $filename_upload_path = $wp_upload_dir['path'] . '/' . basename($filename);
                    copy($filename, $filename_upload_path);
                    $filetype = wp_check_filetype(basename($filename), null);
                    ...
                    $attach_data = wp_generate_attachment_metadata($attach_id, $filename_upload_path);
                    wp_update_attachment_metadata($attach_id, $attach_data);
                    update_woocommerce_term_meta($term['term_id'], 'thumbnail_id', absint($attach_id));
                }
                ...
                foreach ($ids as $id) {
                    ...
                    wp_set_object_terms($post_id, $product_cat_ids_front_page, 'product_cat');
