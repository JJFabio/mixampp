...
if (version_compare(PARENT_THEME_VERSION, '3.1.0', '<')) {
    ...
    //* Footer credits
    add_filter('genesis_footer_creds_text', 'asitheme_genesis_footer_creds_text');

    function asitheme_genesis_footer_creds_text($creds_text) {

        $text = get_theme_mod(ASITHEME_SLUG . '_footer_text', CHILD_THEME_DEFAULTS[ASITHEME_SLUG . '_footer_text']);
    ...
    function asitheme_after_switch_theme() {
        if (!genesis_get_option('footer_text')) {
            genesis_update_settings(
                [
                    'footer_text' => '[' . ASITHEME_SLUG . '_footer_copyright]',
                ]
            );
        }
    }
...
function asitheme_genesis_markup_title_area_close($close_html, $args) {
    ...
    $header_search = get_theme_mod(ASITHEME_SLUG . '_header_search', CHILD_THEME_DEFAULTS[ASITHEME_SLUG . '_header_search']);
