...
include_once(ASITHEME_CDN_PATH . '/inc/widgets/inc/class-asi-widget-main.php');
...
function asi_register_widgets() {
    ...
    genesis_register_sidebar(array(
        'id' => 'asi-front-page',
        'name' => __('Homepage', ASITHEME_SLUG),
    ));

    //* Add support for before entry widget
    genesis_register_sidebar(array(
        'id' => 'asi-before-entry',
        'name' => __('Before entry', ASITHEME_SLUG),
        'description' => __('Widgets in this widget area will display before single entries.', ASITHEME_SLUG)
    ));
...
function asitheme_widgets_wp_enqueue_scripts() {
    ...
    wp_enqueue_script('asitheme-widgets', ASITHEME_CDN . '/assets/js/widgets.js', array(), CHILD_THEME_VERSION, true);
}
...
function asitheme_widgets_wp_head() {
    ...
    <script type="text/javascript">
        var asitheme_magnific_tCounter = '<?php echo esc_js(__('%curr% of %total%', ASITHEME_SLUG)); ?>';
    </script>
    <?php
}
...
function asitheme_add_nav_menu_meta_boxes() {
    ...
    if ($sidebars_widgets['asi-front-page']) {
        add_meta_box('asitheme_nav_menu_links_widgets', sprintf(__('%s Widgets links', ASITHEME_SLUG), CHILD_THEME_AUTHOR), 'asitheme_nav_menu_links_widgets', 'nav-menus', 'side', 'low');
    }
}
