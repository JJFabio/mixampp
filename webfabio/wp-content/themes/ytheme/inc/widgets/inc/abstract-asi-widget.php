...
abstract class ASI_Widget extends WP_Widget {
    ...
    public function admin_head_widgets() {
        wp_enqueue_editor();
        wp_enqueue_media();
        wp_enqueue_script('asitheme-widgets-admin', ASITHEME_CDN . '/assets/js/widgets-admin.js', array('jquery'), CHILD_THEME_VERSION);
        wp_enqueue_style('asitheme-widgets-admin', ASITHEME_CDN . '/assets/css/widgets-admin.css', array(), CHILD_THEME_VERSION);
    }
    ...
    public function asi_form_field_image($value, $key, $setting, $k = '') {
        ...
        <div class="asi-widget-p asi-field-<?php echo esc_attr($setting['type']) ?>" data-field_id="<?php echo esc_attr($key); ?>">
            ...
            <input
                    ...
                    value="<?php _e('Upload Image', ASITHEME_SLUG) ?>">
            <input
    ...
    public function asi_form_field_repeater($value, $key, $setting, $items) {
        ...
        <p>
            <input
                    ...
                    onclick="asitheme_widget_repeater_add(this);"
                    value="<?php _e('Add') ?>">
        </p>
        <script type="text/javascript">
            var asitheme_repeater_item = "<?php echo addslashes(trim(asitheme_minify_output($this->show_item_repeater($key, $items, array())))) ?>";
            ...
            <div class="asitheme-widget-inside">
                <div class="asitheme-widget-content">
                    ...
                    foreach ($items as $k => $item) {
                        ...
                        $this->$function($value[$k], $key, $item, "[$k][]");
                    }
                    ?>
                </div>
            </div>
